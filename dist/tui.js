            ...
            onSelect: async () => {
                ...
                api.ui.dialog.replace(() => api.ui.DialogSelect({
                    ...
                    options: list.map((s) => ({
                        ...
                        value: s.id,
                        description: s.status === "processing"
                            ? "Procesando…"
        ...
        api.slots.register({
            slots: {
                sidebar_footer: (props) => {
                    const related = sessions.filter((s) => s.parentID === props.session_id);
                    if (related.length === 0)
                        return null;
                    ...
                    try {
                        ...
                        const { h } = require("solid-js");
                        return h("div", { style: { padding: "0 8px" } }, [
                            h("div", { style: { "font-weight": "bold", "margin-top": "4px" } }, "🔀 BTW"),
                            ...related.map((s) => h("div", {
                                style: { cursor: "pointer" },
                                onClick: () => api.route.navigate("session", {
                                    sessionID: s.id,
                                }),
